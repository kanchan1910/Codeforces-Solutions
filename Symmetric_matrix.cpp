...
/*one tile will work for me so if even one tile fullfilles the condition answer will be yes and no only if no tile fullfulls ....and we are only checking for b c not for a d becuase i==j is there so s[i][j] will always be equal to s[j][i].SO no need to check it.
