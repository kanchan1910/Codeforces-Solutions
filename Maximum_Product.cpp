...
int main(){
    ...
    lli ans1=arr[0]*arr[1]*arr[2]*arr[3]*arr[4];//all negative so to product will be negtive so selecting min elements
    lli ans2=arr[0]*arr[1]*arr[2]*arr[3]*arr[4];//all 5 positive we can select so selecting max ones to get max product
    lli ans3=arr[0]*arr[n-1]*arr[n-2]*arr[n-3]*arr[n-4];//4 negative and 1 positive ,so positive is the max element and negative will be the last ones
    lli ans4=arr[0]*arr[1]*arr[2]*arr[n-1]*arr[n-2];//2 negative and 3 positive ,so positive is the max element and negative will be the last ones
    lli val=max(ans1,ans2);
    lli val1=max(ans3,ans4);
    lli val2=max(val,val1);
    cout<<(val2)<<endl;
    }
