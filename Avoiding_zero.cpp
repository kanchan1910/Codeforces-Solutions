...
#define ll int256_t
100000 so your total=total*(total-1) can be ~ 10^20 */
...
/*First of all, notice that if the sum a1+a2+⋯+an is 0, then, since b is a rearrangement of a, it holds b1+b2+⋯+bn=0 and therefore the answer is NO.

On the other hand, if a1+a2+⋯+an≠0, then there is a valid array b. To show this, let us consider two cases.

If a1+a2+⋯+an>0, then b can be chosen as the array a sorted in decreasing order. In this way, for any k=1,…,n, it holds b1+⋯+bk>0. Let us prove it by dividing in two cases.
If bk>0, then also b1,…,bk−1 are positive and therefore the sum is positive.
If bk≤0, then also bk+1,…,bn are nonpositive and therefore
b1+⋯+bk=b1+⋯+bn−(bk+1+⋯+bn)≥b1+⋯+bn>0.
If a1+a2+⋯+an<0, then b can be chosen as the array a sorted in increasing order. The proof that this choice works is analogous to the previous case.
*/
