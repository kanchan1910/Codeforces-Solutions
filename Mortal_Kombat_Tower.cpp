    ...
    int fun(int i, int turn){
        ...
        if(turn==1){
            ...
            if(i<n && arr[i]==1){
             ...
             mini=min(mini,fun(i+1,turn^1)+1);   
            }
            if(i<n && arr[i]==0){
               mini=min(mini,fun(i+1,turn^1));   
            }
            if(i+1<n && arr[i]==0 && arr[i+1]==1){
                mini=min(mini,fun(i+2,turn^1)+1);  
            }
              
            if(i+1<n && arr[i]==0 && arr[i+1]==0){
              mini=min(mini,fun(i+2,turn^1));
            }
            if(i+1<n && arr[i]==1 && arr[i+1]==0){
              mini=min(mini,fun(i+2,turn^1)+1);   
            }
            if(i+1<n && arr[i]==1 && arr[i+1]==1){
               mini=min(mini,fun(i+2,turn^1)+2);   
            }
            
            return f[i][turn]=mini;
        }
        else{
            return f[i][turn]=min(fun(i+1,turn^1),fun(i+2,turn^1));;
        }
    }
