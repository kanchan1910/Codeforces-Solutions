...
//main function here
int main(){
    /*We need represent integer number N
    as a sum of maximum possible number of prime numbers, 
    they don’t have to be different.
If N is even number, we can represent it as sum of only 2 - minimal prime number.
It is minimal prime number, so number of primes in sum is maximal in this case.
