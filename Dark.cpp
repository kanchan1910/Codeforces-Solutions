...
void update(lli index,lli val,lli bitarr[],lli n){
    index=index+1;
    while(index<=n){
        bitarr[index]+=val;
        index+=(index&(-index));
...
lli sum(lli index,lli bitarr[],lli n){
    index=index+1;
    lli s=0;
    while(index>0){
        s+=bitarr[index];
...
lli *built(lli arr[],lli n){
    lli *bitarr=new lli[n+1];
    for(lli i=1;i<+n;i++){
        bitarr[i]=0;
    }
    for(lli i=0;i<n;i++){
        update(i,arr[i],bitarr,n);
    }
    return bitarr;
}
//main function here
int main(){
   ...
   while(q--){
       ...
       if(str=="past"){
           ...
           cin>>l>>r;
           arr[r-1]+=l;
           update(r-1,l,bitarr,n);
       }
       else{
           int l,r;
           cin>>l>>r;
           cout<<(sum(r-1,bitarr,n)-sum(l-2,bitarr,n))<<endl;
       }
   }
}